...
def register_page():
    ...
    if form.validate_on_submit():
        user_exists = User.query.filter_by(username = forml.username.data).first() #get username data entered from sign in form
        if user_exists:
            flash("Username already exists!")
            return render_template('signup.html', form = form, forml=forml)
        else:
